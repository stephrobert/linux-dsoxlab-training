...
def test_service_no_recent_failures(host):
    """Vérifie que le service n'a pas eu de failure dans la dernière minute.

    Le journalctl recent ne doit plus contenir le pattern de la cause
    initiale (`Configuration file not found`).
    """
    out = host.run(
        f"journalctl -u {SERVICE_NAME} --since '1 minute ago' --no-pager 2>/dev/null"
    )
    assert "Configuration file not found" not in out.stdout, (
        f"Le journal récent contient encore le crash loop initial. "
        f"Le service crash-il toujours ? Vérifiez `systemctl status {SERVICE_NAME}`."
    )
